...
製品の判断基準は[SPEC §1](SPEC.md#1-現行製品定義)、画面とAPIの技術仕様は[Signal UI実装](docs/architecture/signal-ui.md)。対応Issueの最新指示から作業を継続し、製品定義、branch実装、検証したデータ・モード、公開状態を区別する。変更が解決する利用者の問い、対応Issue、画面・実行結果の証拠、未達の価値を対応付け、技術検証だけで業務上の価値を受入済みとしない。

§1〜12のPhase 0固有の仕様・手順・完了条件は従来の`standard`画面に限定する。静的配信のみ・Backendなし・Web/LLMなし・①〜⑤固定という制約を`signals`画面への禁止として適用しない。現在のサービスの技術上の境界は§14に従い、旧画面と既存ルール分析の動作を保つ。

最新状態と証拠は対応IssueとPRを正本とする。[Frontend移行設計](docs/architecture/google-cloud-frontend.md)とSPEC §7のIssue #15時点の設計・制約は履歴として参照する。過去の文書作業だけという制約を後続Issueの許可済み実装に重ねず、DraftのUIやAPIをmain・公開版の実装済みと記載しない。push・クラウド・認証変更・main/Productionの承認境界は対応Issueと§13に従う。

## 1. 役割と最優先原則
あなた（Codex）は本リポジトリの実装担当。以下を順守する。
1. **`SPEC.md` §1と対応Issueのスコープに従う。** 機能を勝手に増やさない。従来のPhase 0 `standard`画面はSPEC §2を適用し、WEB/プレス/新聞/株主総会を「準備中」に保つ。
2. **軽く・安く。** Phase 0は静的ホスティング（GitHub Pages）で動き、常時稼働の有料バックエンドに依存しない。新段階も不要なサービスや依存を増やさない。
3. **Phase 0のstandard画面はルールベース分析で完結させる。** この経路にLLM連携、APIキー入力、`USE_LLM` による切替を実装しない。
4. **まず動くものを通しで。** 対応Issueの利用者の操作から結果・根拠の確認までを通し、Phase 0の`standard`動作を保つ。
...
- ❌ Phase 0のWEB/プレス/新聞/株主総会のデータ取得を実装する（準備中固定）。新段階の指定公式HTML照合も、別Issueで実装されるまでUIを有効化しない。
- ❌ Phase 1 の一括データ処理をブラウザ内で実行する。
- ❌ スコープ外機能の追加、重い依存の導入。Phase 0のstandard画面を常時稼働の有料バックエンド前提に変更すること。
...
## 14. Google Cloudサービスの技術上の境界

- 対応するFrontend Issueと親Issueの合意範囲に従う。公開文書へprivate Backendの内部構成、環境識別子、実資料・実画面、データ、hash/path、認証情報を転載しない。
- Vite/Reactを維持する。第一案はCloud Runによるビルド済SPAとAPIの同一origin配信。DBはCloud SQL for PostgreSQL、原本・画像・exportはCloud Storage、補足AIはVertex AI上のGeminiをBackendが扱う。理由なくNext.js、別BFF、API Gatewayを追加しない。
- Frontendは認証されたAPIへの要求と安全な表示に限定する。DB接続、保存先内部location、モデル呼出、Google認証情報はBackendに閉じ込める。ブラウザへのAPIキー入力・保存UI、SA JSON key、秘密の`VITE_*`変数は禁止する。
- BackendのローカルADC／Cloud Run service account + IAMはGoogle API用の実行主体であり、利用者ログインの代わりにはならない。移行前後の利用者認証と未認証・誤認証の拒否を別途確認する。
- `RuleBasedAnalysisEngine`、決定的な件数・条件・stable evidence ID・品質判定を維持する。Draft系列のContract Adapterはexact `0.1.0`検証とaccepted-only分析を保ったまま、別承認の統合作業で引き継ぐ。
- Geminiの補足結果と公式出典は別のversioned DTOで扱い、既存Contract `0.1.0`へ黙って追加しない。既存ルール結果を`generatedBy: 'llm'`へ書き換えず、AI障害時もルール結果を利用可能にする。
- 登録済みの企業・商品分野・比較組・公式資料を対応Issueの範囲で扱う。事実／AI仮説／不明点と出典・日付・根拠意匠を分離し、保存結果の再表示でAIを再実行しない。SPEC §7の初回HTML1件は過去の設計案である。
- 正式サービスは固定URLでPC停止時も利用できる構成とし、自動期限・失効URLやTunnel公開を採用しない。既存trialの承認済期限は別物として、最新承認履歴とlive状態を照合し、対応Issueの許可なく変更しない。
- 旧配信の停止は、新経路の主要操作・認証拒否・永続保存結果の再表示・PC停止試験を確認した後の別承認とする。既存アプリや保存データの削除は別の判断・承認に従う。Draft解除、merge、Production反映の承認境界は§13を維持する。
