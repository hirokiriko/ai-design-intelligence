...
## 0. 段階と適用範囲（2026-09-19）

Frontend Issue #17 / Backend Delivery #15は、保存条件、差分、画像AI観察候補、登録公式サイト内の有限探索、根拠付きシグナルと保存履歴の実装を許可する。詳細は[Signal UI実装](docs/architecture/signal-ui.md)。以下のIssue #15文書のみという制約は、その過去の文書作業の範囲であり、Issue #17の許可済み実装を妨げない。push・クラウド・認証変更・main/Productionの承認境界は親Deliveryに従う。

§1〜12は既存のPhase 0サンプル動作の規約として残す。静的配信のみ・常設Backendなし・Web/LLMなしという制約を、新段階全体への恒久禁止とはしない。正式サービスの新方針は§14と[SPEC §7](SPEC.md#7-google-cloud正式サービス新方針未実装)に従う。

現在のmain、未マージDraft、新方針を区別する。詳細と確認時点は[Frontend移行設計](docs/architecture/google-cloud-frontend.md)を正本とし、DraftのUIやAPIをmain実装済みと記載しない。Issue #15は文書のみであり、新機能実装・認証変更・クラウド作成・デプロイを許可しない。

## 1. 役割と最優先原則
あなた（Codex）は本リポジトリの実装担当。以下を順守する。
1. **`SPEC.md` §2 のPhase 0スコープを逸脱しない。** 機能を勝手に増やさない。Phase 0のWEB/プレス/新聞/株主総会は実装せず「準備中」固定。
...
## 14. Google Cloud正式サービスの規約（設計、未実装）

- [Frontend Issue #15](https://github.com/hirokiriko/ai-design-intelligence/issues/15)と、そこから参照する親Issueの合意範囲に従う。公開文書へprivate Backendの内部構成、環境識別子、データ、hash/path、認証情報を転載しない。
...
- 初回は1企業・1商品分野・指定企業公式HTML1件の照合導線だけを設計する。事実／AI仮説／不明点と出典・日付・根拠意匠を分離し、保存結果の再表示でAIを再実行しない。
- 正式サービスは固定URLでPC停止時も利用できる構成とし、自動期限・失効URLやTunnel公開を採用しない。既存trialの承認済期限は別物として、最新承認履歴とlive状態を照合し、この文書作業では変更しない。
- 旧配信の停止は、新経路の主要操作・認証拒否・永続保存結果の再表示・PC停止試験を確認した後の別承認とする。本Issueでは既存アプリや保存データを削除せず、削除は別の判断・承認に従う。Draft解除、merge、Production反映の承認境界は§13を維持する。
