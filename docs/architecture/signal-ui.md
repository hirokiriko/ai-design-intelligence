...
新規保存結果には補足DTO `2.0.0` を使用する。公開契約は `contracts/run-v2.0.0.schema.json` と `contracts/signal-v2.0.0.schema.json`。既存の補足 `1.0.0` の保存結果と Contract `0.1.0` の分析経路は維持する。未知version・未知field・参照不一致では表示を停止する。
...
現在の状態：架空fixtureによるv1/v2混在履歴、表示分岐、対応区分、日付・抜粋・参照不一致の単体回帰を追加した実装候補。次のアクション：最新候補の実APIによる保存・履歴・reload、データ更新後の別実行を統合確認する。架空試験を承認済み実企業ケースの実測完了とは扱わない。

## Issue #17で引き継いだ導線

親DeliveryはBackend #15。画面はVite/React、既存ルール分析は`generatedBy: rules`、元データはContract `0.1.0`を維持する。初期実装の保存条件と保存結果は補足API `1.0.0`を使用していた。保存済みの旧形式runは架空運用のもので、新形式の環境区分で書き換えない。新規実行の契約は上記Issue #19の`2.0.0`を参照する。
