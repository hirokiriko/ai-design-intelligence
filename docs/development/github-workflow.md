...
- 合意済みの作業では、GitHubへ書き込む直前に対象リポジトリと実施内容を短く明示し、Issueを直接作成・更新する。
- 未合意、仕様が曖昧、または影響が大きい作業では、Issueタイトルと本文の最新版全文をユーザーへ提示し、明示承認後に作成・更新する。
- ユーザーが自分でコピーする文章は、差分ではなくそのまま使える最新版全文を提示する。
...
- 未合意、仕様が曖昧、または影響が大きいIssueの作成・更新
- Draft解除
- mainへのマージ
- Production反映
- 実データ投入や認証境界の変更
...
3. 作業が合意済みか、未合意・仕様が曖昧・影響が大きいかを判断する。
4. 合意済みの作業では、GitHubへ書き込む直前に対象リポジトリと実施内容を短く明示し、Issueを直接作成・更新する。
5. 未合意、仕様が曖昧、または影響が大きい作業では、Issueタイトルと本文の最新版全文をユーザーへ提示し、明示承認後に作成・更新する。
6. ユーザーが自分でコピーする文章は、差分ではなくそのまま使える最新版全文を提示する。
...
Issue作成時にローカル未追跡状態を確認できない場合は、推測や古い件数で補完せず、`未確認（Codexが作業開始時と終了時に確認）`と記載できる。Codexは作業開始時に実際の状態を確認し、Issue記載との差異があればPRへ記録する。
