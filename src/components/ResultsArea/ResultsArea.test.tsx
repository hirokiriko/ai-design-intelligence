...
const demoShowcaseRecords: DemoShowcaseRecord[] = [
  {
    ...
    drawingLabels: ['画像図'],
    sourceXmlFile: 'sample-publication-meta-fixture-001.xml',
    whyDemoFriendly: '図面情報あり / 物品名が説明しやすい',
  },
  {
    id: 'fixture-without-keys',
    articleName: 'サンプル収納ケース',
...
describe('ResultsArea gazette drawing metadata display', () => {
  it('renders only allowed gazetteDrawingKeys fields and no image body, URL, or local full path', () => {
    ...
    expect(resultOrder.every((position) => position >= 0)).toBe(true);
    expect(resultOrder).toEqual([...resultOrder].sort((left, right) => left - right));
    expect(html).toMatch(/<button[^>]*type="button"[^>]*>[\s\S]*?1\. 今回の分析対象意匠数[\s\S]*?2件[\s\S]*?クリックして対象意匠を確認[\s\S]*?<\/button>/);
    expect(html).toContain('対象意匠件数: 2件 ／ 根拠意匠2件を見る');
    expect(html).toContain('画像意匠件数: 1件 ／ 根拠意匠1件を見る');
    expect(html).toContain('対象企業数: 2社 ／ 根拠意匠2件を見る');
    ...
    expect(html).toContain('公報・図面情報あり');
    expect(html).toContain('図面情報あり：2件中1件');
    ...
    expect(html).toContain('この意匠には、取得済みの公報・図面情報がありません。');
    ...
    expect(html).toContain('データ更新時点');
    expect(html).toContain('図面画像本体と外部公報リンクは未接続です。');
  ...
  it('renders external demo mode guide, showcase records, and folded unresolved codes safely', () => {
    ...
    expect(html).toContain('5分デモ');
    expect(html).toContain('0:00–0:40 目的');
    expect(html).toContain('0:40–1:30 条件設定');
    expect(html).toContain('1:30–3:00 分析結果');
    expect(html).toContain('3:00–4:10 根拠確認');
    expect(html).toContain('4:10–5:00 展開可能性と現在地');
    expect(html).not.toContain('3分デモ');
    expect(html).not.toContain('10分デモ');
    expect(html).toContain('デモシナリオ導線');
    expect(html).toContain('今どこを説明しているか');
    expect(html).toContain('次に見る場所');
    expect(html).toContain('意匠情報から、企業各社や特定他社がどの領域に着目しているか、商品開発傾向・デザイン変化・出願活動の兆候を読むための参考情報にします。');
    expect(html).toContain('デモで見るポイント');
    expect(html).toContain('このアプリの強み');
    expect(html).toContain('意匠情報から、企業各社や特定他社がどの領域に着目しているか、商品開発傾向・デザイン変化・出願活動の兆候を読む');
    expect(html).toContain('意匠情報を主対象に分析できます');
    expect(html).toContain('他の知財情報や事業情報との照合は別途検討します');
    ...
    expect(html).toContain('図面情報ありの根拠だけ表示');
    expect(html).toContain('図面名が確認できる根拠意匠に絞ります。');
    expect(html).toContain('A. 見せ場サマリー');
    expect(html).toContain('公報・図面情報あり');
    expect(html).toContain('図面名の代表3件');
    expect(html).toContain('B. 基本情報');
    expect(html).toContain('C. 公報・図面情報');
  ...
  it('shows the first eight evidence records before offering the remaining records', () => {
    ...
    expect(html.match(/data-testid="priority-insight"/g)).toHaveLength(1);
    expect(html).toContain('対象意匠件数: 9件 ／ 根拠意匠9件を見る');
  ...
  it('explains how to recover when no records match the selected conditions', () => {
    ...
    expect(html).toContain('現在のデータと条件では検出されませんでした。');
    expect(html).toContain('企業名、見たい領域、期間、意匠情報の種類を見直して');
