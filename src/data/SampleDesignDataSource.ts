...
export class SampleDesignDataSource implements DesignDataSource {
  ...
  getAllRecords(): ProjectedLegacyDesignRecord[] {
    return [...sampleAnalysisRecords].sort(compareSampleRecords);
  }

  getViewRecords(): DesignRecord[] {
    return [...sampleDataset.records].sort(compareSampleRecords);
  }
}
...
function compareSampleRecords(
  left: Pick<AnalysisReadyDesignRecord, 'gazetteDate' | 'id'>,
  right: Pick<AnalysisReadyDesignRecord, 'gazetteDate' | 'id'>,
): number {
  return right.gazetteDate.localeCompare(left.gazetteDate) || left.id.localeCompare(right.id, 'en');
}
