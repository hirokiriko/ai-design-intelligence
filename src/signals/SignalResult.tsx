...
const evidenceId = (id: string) => `signal-evidence-${encodeURIComponent(id)}`;

function EvidenceLinks({ ids }: { ids: string[] }) {
  return <span className="signal-evidence-links">根拠：{ids.map((id) => <a key={id} href={`#${evidenceId(id)}`}>{id}</a>)}</span>;
}
...
export function SignalResult({ run, subject }: { run: Run; subject?: string }) {
  ...
  return <article className="signal-result" aria-labelledby="signal-result-heading">
    ...
    {run.versions.model === 'fixture-controller' ? <p className="signal-data-banner">模擬モデル（接続・保存の検証） · 実際の画像AI・公式サイト取得の評価結果ではありません。</p> : null}
    ...
    {signal ? <>
      ...
      <section className="signal-summary-block"><h3>次に確認すべきこと</h3>{signal.questionsForHuman.length ? <ul>{signal.questionsForHuman.map((text, index) => <li key={index}>{text}</li>)}</ul> : <p>追加の確認事項は登録されていません。</p>}</section>
      <section className="signal-facts"><h3>意匠データの事実</h3>{signal.designFacts.length ? signal.designFacts.map((fact) => <div className="signal-fact" id={evidenceId(fact.id)} key={fact.id} tabIndex={-1}><p>{fact.text}</p><p className="signal-subtle">項目：{fact.field}</p>{fact.recordIds.map((recordId) => <SignalRecordEvidence key={recordId} recordId={recordId} watch={run.input.watch} />)}</div>) : <p>今回の範囲では、追加の事実を確認できませんでした。</p>}</section>
      ...
      <section><h3>公式発表の事実</h3>{signal.officialFacts.length ? signal.officialFacts.map((fact) => <div className="signal-fact" id={evidenceId(fact.id)} key={fact.id} tabIndex={-1}><p>{fact.text}</p><blockquote>{fact.quote}</blockquote><EvidenceLinks ids={[fact.sourceId]} /></div>) : <p>対象範囲で、根拠として採用できる公式発表を確認できませんでした。</p>}</section>
      <section className="signal-hypotheses"><h3>関連仮説 · 未確認</h3><p className="signal-subtle">意匠と公式資料の関係は仮説です。公式に同一製品や商品化を確認したことを意味しません。</p>{signal.hypotheses.length ? signal.hypotheses.map((item) => <div className="signal-fact" key={item.id} id={evidenceId(item.id)} tabIndex={-1}><p>{item.text}</p><EvidenceLinks ids={item.evidenceIds} /><ul>{item.limitations.map((text, index) => <li key={index}>{text}</li>)}</ul></div>) : <p>根拠のある関連仮説はありません。</p>}</section>
      ...
      {signal.sources.length ? <section><h3>確認した公式資料</h3>{signal.sources.map((source) => <article id={evidenceId(source.id)} key={source.id} className="signal-source" tabIndex={-1}><h4><a href={source.url} target="_blank" rel="noopener noreferrer">{source.title || 'タイトル不明'} ↗</a></h4><p className="signal-subtle">公開日：{source.publishedAt ?? '不明'} · 取得日時：{source.retrievedAt}</p>{source.retrospective ? <p className="signal-retrospective">事後照合：意匠の基準日より後の資料を含みます。</p> : null}<blockquote>{source.excerpt || '抜粋なし'}</blockquote><p className="signal-subtle readable-text">{source.url}</p></article>)}</section> : null}
