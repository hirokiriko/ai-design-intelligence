...
import { evidenceId, factsOnlyRun, runLabels } from './labels';
...
export function SignalResult({ run }: { run: Run }) {
  ...
  return <article className="signal-result" aria-labelledby="signal-result-heading">
    ...
    {signal ? <>
      ...
      {signal.sources.length ? <section><h3>{factsOnly ? '手動確認した公式資料のリンク' : '確認した公式資料'}</h3>{signal.sources.map((source) => <SourceCard key={source.id} source={source} run={run} />)}</section> : null}
