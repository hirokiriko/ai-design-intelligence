...
export function SignalResult({ run }: { run: Run }) {
  ...
  return <article className="signal-result" aria-labelledby="signal-result-heading">
    ...
    {signal ? <>
      ...
      <section className="signal-hypotheses"><h3>関連仮説 · 未確認</h3><p className="signal-subtle">{run.schemaVersion === '2.0.0' ? '関連性についての仮説とその限界です。個別製品との対応の確認状況は「個別意匠と商品の対応」で確認してください。' : '意匠と公式資料の関係は仮説です。公式に同一製品や商品化を確認したことを意味しません。'}</p>{signal.hypotheses.length ? signal.hypotheses.map((item) => <div className="signal-fact" key={item.id} id={evidenceId(item.id)} tabIndex={-1}><p>{item.text}</p><EvidenceLinks ids={item.evidenceIds} /><ul>{item.limitations.map((text, index) => <li key={index}>{text}</li>)}</ul></div>) : <p>根拠のある関連仮説はありません。</p>}</section>
