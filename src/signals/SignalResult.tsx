...
import { evidenceId, factsOnlyRun, runStatusLabel } from './labels';
...
function SourceCard({ source, run }: { source: Signal['sources'][number]; run: Run }) {
  ...
  const manualFact = factsOnlyRun(run) && v2Source?.extractionVersion === 'manual-public-fact-1.0.0';
  return <article id={evidenceId(source.id)} className="signal-source" tabIndex={-1}>
    <h4><a href={source.url} target="_blank" rel="noopener noreferrer">{source.title || 'タイトル不明'} ↗</a></h4>
    ...
    {manualFact ? <p className="signal-subtle">手動確認した出典リンクです。記事本文・画像は保存せず、AIへ送信していません。</p> : v2Source ? <>
    ...
    <p className="signal-subtle">出典リンクは別タブで開きます。根拠の表示でAI・外部ページ取得を自動実行しません。</p>
  </article>;
}
...
export function SignalResult({ run }: { run: Run }) {
  ...
  return <article className="signal-result" aria-labelledby="signal-result-heading">
    ...
    {factsOnly ? <p className="signal-data-banner">書誌事項の決定的な比較です。実行時に記事本文・図面を取得せず、AI・Vertexへ送信していません。出典リンクは手動確認用です。</p> : <p className="signal-subtle">AIモデル：{run.versions.model}。モデルの種類と、意匠・画像・資料が実在するかどうかは別の情報です。</p>}
    ...
    {signal ? <>
      ...
      <section className="signal-hypotheses"><h3>関連仮説 · 未確認</h3><p className="signal-subtle">{factsOnly ? '記事本文と図面を分析していないため、意匠と商品の関係は評価していません。' : run.schemaVersion !== '1.0.0' ? '関連性についての仮説とその限界です。個別製品との対応の確認状況は「個別意匠と商品の対応」で確認してください。' : '意匠と公式資料の関係は仮説です。公式に同一製品や商品化を確認したことを意味しません。'}</p>{signal.hypotheses.length ? signal.hypotheses.map((item) => <div className="signal-fact" key={item.id} id={evidenceId(item.id)} tabIndex={-1}><p>{item.text}</p><EvidenceLinks ids={item.evidenceIds} /><ul>{item.limitations.map((text, index) => <li key={index}>{text}</li>)}</ul></div>) : <p>根拠のある関連仮説はありません。</p>}</section>
      ...
      {signal.sources.length ? <section><h3>{factsOnly ? '手動確認した参照リンク' : '確認した公式資料'}</h3>{signal.sources.map((source) => <SourceCard key={source.id} source={source} run={run} />)}</section> : null}
    ...
    <details className="signal-details"><summary>保存条件・実行情報</summary><dl className="signal-metadata"><div><dt>run ID</dt><dd>{run.id}</dd></div><div><dt>企業ID / 分類ID</dt><dd>{run.input.watch.entityId} / {run.input.watch.categoryId}</dd></div><div><dt>比較A / 比較B</dt><dd>{run.input.watch.beforeDatasetId} / {run.input.watch.afterDatasetId}</dd></div><div><dt>登録した参照先</dt><dd>{run.input.watch.sourceProfileId}</dd></div><div><dt>AI呼出 / 確認処理</dt><dd>{run.usage.modelRequests} / {run.usage.toolCalls}</dd></div></dl><p className="signal-subtle">保存履歴の再表示・再読み込みではAIを呼び出しません。過去の結果は上書きしません。</p></details>
  </article>;
}
